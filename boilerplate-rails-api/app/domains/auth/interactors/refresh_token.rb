  ...
  module Interactors
    class RefreshToken
      ...
      def call(user_id:, raw_refresh_token:)
        ...
        { access_token: JwtService.encode_access_token(user_id: user.id, email: user.email, super_admin: user.super_admin, permissions: permissions), refresh_token: new_raw_refresh }
      end
    end
  end
end
