  ...
  module Interactors
    class LoginUser
      ...
      def call(email:, password:)
        ...
        { user: @repo.find_by_id(user.id), access_token: JwtService.encode_access_token(user_id: user.id, email: user.email, super_admin: user.super_admin, permissions: permissions), refresh_token: raw_refresh }
      end
    end
  end
end
